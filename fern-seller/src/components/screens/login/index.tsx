...
import AuthShape from './auth-shape';
import styles from './auth.module.scss';

export enum LoginFormTargets {
...
interface Props {
  action: (target: LoginFormTargets, value?: LoginFormValues) => void;
}
  ...
  return (
    <div className={styles.container}>
      <div className={styles.content}>
        <div className={styles.shapesBX}>
          <div className={styles.shapesHD}>
            <div>{Logo}</div>
            <div>به مرکز فروشندگان السل خوش آمدید</div>
          </div>
          <div className={styles.shapesBD}>
            <AuthShape />
          </div>
        </div>
        <div className={styles.authBX}>
          <Login
            mainExplanation={'ورود به بخش فروشندگان'}
            username={{
              placeHolder: 'ایمیل',
              value: email,
            ...
            rememberMe={{ state: isRememberChecked, onChange: (e) => setIsRememberChecked(e) }}
            button={{
              title: 'ادامه',
              onClick: () => onLoginClicked(email, password, isRememberChecked, props),
            }}
            forgotPassword={{
...
const passwordValidator = new TextValidator(ValidatorScope.password);
...
const onLoginClicked = (email: string, password: string, isRememberChecked: boolean, props: Props): void => {
  if (isValid(email, password)) {
    props.action(LoginFormTargets.SUBMIT, {
      email,
      password,
      isRememberChecked,
    });
